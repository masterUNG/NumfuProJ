  ...
  void initState() {
    ...
    AppService().findPostion().then((value) {
      ...
      controller.distanceKm.value = AppService().calculateDistance(
          controller.positions.last.latitude,
    ...
    return Scaffold(
      ...
      body: GetX(
          ...
          builder: (AppController appController) {
            print(
                '## sqliteModels at menu ---> ${appController.sqliteModels.length}');
            return Padding(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          ShowTitle(
                              title:
                                  ' ${appController.distanceKm.toString()} km',
                              textStyle: MyCostant().h3Style())
                        ],
                      ),
                      Row(
                        children: [
                          Text('ค่าใช้จ่าย :', style: MyCostant().h3Style()),
                          ShowTitle(
                              title:
                                  '${AppService().calculatePriceDistance(distance: appController.distanceKm.value)} thb',
                              textStyle: MyCostant().h3Style())
                        ],
                      ),
    ...
    return GestureDetector(
      onTap: () {
        SQLiteHelper().readData().then((value) {
          ...
          print(
              '##16april ขนาดของ SQLite ที่มี --> ${appController.sqliteModels.length}');

          print('##16april sqliteModel ที่จะ add ---> ${sqLiteModel.toMap()}');
          ...
          if (appController.sqliteModels.isEmpty) {
            ...
            print(
                '## appController.sqLModels.last --> ${appController.sqliteModels.last.toMap()}');
